      ...
      <div className="aa-modal flex max-h-[80vh] w-full max-w-2xl flex-col rounded-md shadow-2xl">
        <div className="aa-modal-header flex items-center justify-between border-b p-6">
          <h2 className="aa-title flex items-center gap-2 text-xl font-bold">
            <History className="aa-linkish w-5 h-5" />
            업데이트 내역
          </h2>
        ...
        <div className="aa-muted overflow-y-auto p-6 leading-relaxed custom-scrollbar">
          <div className="relative ml-3 space-y-8 border-l border-[var(--aa-border)]">
            {changelogData.map((ver, idx) => (
              <div key={idx} className="mb-8 ml-6 relative group">
                <span className="aa-panel-soft absolute -left-[31px] top-1.5 flex h-4 w-4 items-center justify-center rounded-full transition-colors group-hover:border-[var(--aa-accent)]">
                  <div className="h-1.5 w-1.5 rounded-full bg-[var(--aa-subtle)] transition-colors group-hover:bg-[var(--aa-accent)]"></div>
                </span>
                
                <div className="flex items-baseline justify-between mb-2">
                  <h3 className="aa-title flex items-center gap-2 text-lg font-bold">
                    v{ver.version}
                  </h3>
                  <span className="aa-subtle font-mono text-xs">{ver.date}</span>
                </div>
                
                <ul className="space-y-2">
                  {ver.changes.map((change, cIdx) => (
                    <li key={cIdx} className="aa-muted flex items-start gap-2 text-sm">
                      <GitCommit className="aa-subtle mt-0.5 h-4 w-4 shrink-0" />
                      <span>{change}</span>
                    </li>
                  ))}
                </ul>
